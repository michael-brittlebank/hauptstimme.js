(function(){
                            ...
                            if (dataChords[chordGroup].hasOwnProperty(chordKey)) {
                                ...
                                for (var j = 0; j < chordFormula.length; j++) {
                                    ...
                                    if (typeof rawChordStep === 'string') {
                                        ...
                                        scaleTone = majorScale.tones[that.convertFromScaleDegreeToArrayIndex(rawChordStep)];
                                        adjustedTone = scaleTone;
                                    }
                                    tones.push(adjustedTone);
                                }
                                chordName = rootName + ' ' + helpers.capitalize(chordKey.replace(/_/g, ' '));
                                letters = that.getNotesInChord(rootName, tones);
     ...
     */
    this.searchChords = function(tones, container, searchedNotesAlready){
        return new Promise(function(resolve,reject) {
            ...
            if ((container || !tones) && !searchedNotesAlready) {
                ...
                return that.searchChords(logicNotes.getSelectedNotes(container), container, true);
            } else {
                var chordGroup,
                    chordKey,
                ...
                return events.dispatchEvent(
                    _chorus.data.customEvents.chorusChordSearchComplete,
                    'chorusJS has finished searching chords'
                )
                    .then(function(){
                        resolve(data);
                    });
