...
_chorus.init = _chorus.init || function(element, config){
    ...
    if (element !== undefined) {
        if (document.getElementById(element) !== null) {
            ...
            _chorus.events.messages.sendMessage(_chorus.data.dictionaries.messages.notFound+"no elements found using init value");
            return false;
        }
    }
    else {
        _chorus.events.messages.sendMessage(_chorus.data.dictionaries.messages.undefined+"no element defined in init");
        return false;
    }
};
